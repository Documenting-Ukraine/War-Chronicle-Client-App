...
const RevokeAccessPopUp = ({ onClick, user }: PopUpProps) => {
    return <div className="dashboard-popup-modal revoke-access-pop-up">
        <div></div>
  </div>;
};
const UserScopePopUp = ({ onClick, user, index }: PopUpProps) => {
    return (
        ...
        <div className="edit-user-scope-header">
          ...
          <button data-action-type="close-modal" onClick={onClick}>
            ...
            <div className="edit-user-scope-body">
                <button data-action-type={""} data-index={index}onClick={onClick}>Save Changes</button>
            </div>
      </div>
    );
};
