...
import RequestAccessInput, {CustomRequestAccessInput, customStylesErr} from "../../../forms/requestAccessForm/RequestAccessInput";
...
const AddUserModal = ({
  ...
  const [submitted, setSubmitted] = useState<false | NewInviteLinkPayload[]>(false)
  const { err: accountTypeErr, onTouch, onDropdownChange, value: accountType } = useFormInputs({
    required: true
  });
  const { err: categoryScopeErr, onTouch: onCategoryScopeTouch, onDropdownMultiChange, multiValue: assignedScopes } = useFormInputs({
    required: true,
    isMulti: true
  });
  ...
  const onSendUserInvite = async (e: React.FormEvent<HTMLFormElement>) =>{
    ...
    const email = removeAddedWhiteSpace(fieldValues["New User Gmail"].toString())
    const submitAccountType = removeAddedWhiteSpace(fieldValues["Account Type"].toString())
    if(!validEmail(email)) return
    if(submitAccountType !== "admin" && submitAccountType !== "contributor") return 
        ...
        >
            ...
            {isLoading && (
                ...
                >
                    ...
                    />
                    {accountTypeErr.err && <div className="row-input-error">{accountTypeErr.message}</div>}
                    </>
