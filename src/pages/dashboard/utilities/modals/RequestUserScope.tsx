...
const RequestNewScopesModal = ({closePopUp}:RequestNewScopesProps) => {
    ...
    const onRequestAccessSubmit = async (e: React.FormEvent<HTMLFormElement>) =>{
        ...
        try{
            setIsLoading(true)
            const result = await app.currentUser?.callFunction("request_new_scope", payload)
            if(!isRequestScopeResult(result)) throw new Error('Something went wrong')
        ...
        <PopUpBg fullViewport={true} onClick={onRequestAccess}>
            ...
            {!isSubmitted.submitted &&
                <form onSubmit={onRequestAccessSubmit}>
                    ...
                    >
                        <>
                            {err.err && <div className="request-scope-pop-up-err">{err.message}</div>}
                            {isLoading && <div className="request-scope-pop-up-loading"><LoadingIcon strokeWidth={"0.2rem"}/></div>}
            ...
            {isSubmitted.submitted && <div className="request-scope-pop-up-submitted">{isSubmitted.message}</div>}
            </>
        </PopUpBg>
    )
};
